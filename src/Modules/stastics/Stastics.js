...
function stasticsController($state, $stateParams, $firebaseArray, $firebaseObject) {
    ...
    sprintInfor.$loaded().then(res => {
      ...
      self.chart1 = {
        ...
        series: [{
          innerSize: '65%',
          data: [
            { name: "Média Dev", y: res.mediaDev },
            { name: "Pontos", y: res.pesoTotal }  
          ]
        }]
      }
    })
    ...
    /*   console.table([{name: 'totalMelhoria', valor: totalMelhoria },
      ...
      if (totalPontosSomandos < totalDePontos) {
        ...
        data2.push({name: "Outros", y: totalOutros})
      }

      
      /** GRAFICO 3 **/
      res.forEach(result => {
        data3.push({name: result.nome.split(' ')[0] ,data: [result.total_pontos_dev]})
      })
